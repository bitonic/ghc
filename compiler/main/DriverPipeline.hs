...
-----------------------------------------------------------------------------
-- stub .h and .c files (for foreign export support), and gcc files.
...
compileForeign :: HscEnv -> Phase -> FilePath -> IO FilePath
compileForeign hsc_env phase stub_c = do
        (_, stub_o) <- runPipeline StopLn hsc_env (stub_c, Just (RealPhase phase))  Nothing
                                   Temporary Nothing{-no ModLocation-} []

        return stub_o

compileStub :: HscEnv -> FilePath -> IO FilePath
compileStub hsc_env stub_c = compileForeign hsc_env Cc stub_c
        ...
        case result of
            ...
            HscRecomp cgguts mod_summary
              -> do output_fn <- phaseOutputFilename next_phase
                    ...
                    (outputFilename, mStub, gcc_files) <- liftIO $ hscGenHardCode hsc_env' cgguts mod_summary output_fn
                    stub_o <- liftIO (mapM (compileStub hsc_env') mStub)
                    gcc_os <- liftIO (mapM (uncurry (compileForeign hsc_env')) gcc_files)
                    setForeignOs (maybe [] return stub_o ++ gcc_os)
