...
hscGenHardCode :: HscEnv -> CgGuts -> ModSummary -> FilePath
               -> IO (FilePath, Maybe FilePath, [(Phase, FilePath)]) -- ^ @Just f@ <=> _stub.c is f
hscGenHardCode hsc_env cgguts mod_summary output_filename = do
        ...
        withTiming (pure dflags)
            ...
            (output_filename, (_stub_h_exists, stub_c_exists), gcc_files)
                <- {-# SCC "codeOutput" #-}
                  codeOutput dflags this_mod output_filename location
                  foreign_stubs gcc_files dependencies rawcmms1
            return (output_filename, stub_c_exists, gcc_files)
